...
import cordova.plugin.helloWorld.helpers.*;
import io.ionic.starter.R;

public class SettingsActivity extends Activity {
	public static SharedPreferences settings;
	public Hashtable<String,Setting> settingValues = new Hashtable<String,Setting>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for(int i=0; i<MainActivity.sensors.length; i++) {
			...
			if( s == null ) 
				continue;
			String val = settings.getString("sensor-" + MainActivity.sensors[i], "");
			if( val == null || val.equals("") ) {
				Editor editor = settings.edit();
			    editor.putString("sensor-" + MainActivity.sensors[i], values[i]);
			    editor.commit();
			}
		}
		...
		for(int i=0; i<MainActivity.sensors.length; i++) {
			...
			if( s == null ) 
				continue;
			createSettingField( s.getName(), "sensor-" + MainActivity.sensors[i], tableLayout, tableParams, i + 1 );
		}
		...
		saveButton.setOnClickListener( new View.OnClickListener() {
			...
			public void onClick(View v) {
			   ...
			   for(int i=0; i<MainActivity.sensors.length; i++) {
			    	Setting setting = settingValues.get("sensor-" + MainActivity.sensors[i]);
			    	if( setting != null ) {
				    	editor.putString("sensor-" + MainActivity.sensors[i], setting.getSamplingRate(l) );
				    	editor.putString("sensor-" + MainActivity.sensors[i] + "-enabled", setting.isEnabled(l) );
				    	editor.putString("sensor-" + MainActivity.sensors[i] + "-displayed", setting.isDisplayed(l) );
			    	}
			    }
			    
			    Setting setting = settingValues.get("foregound-application");
	...
	void createSettingField( String name, final String settingName,TableLayout tableLayout, TableLayout.LayoutParams tableParams, int index ) {
		...
		Setting setting = new Setting( settingName, index * 100 + 1, index * 100 + 2, index * 100 + 3 );
